...
def prepare_batch_conditionals(
    ...
    model: 'ChatterboxMultilingualTTS', # Типизация для ясности
    ve: 'VoiceEncoder',
    s3gen: 'S3Gen',
    ...
    # ⚡️ ОПТИМИЗАЦИЯ 1: Извлекаем тензоры на CPU один раз за пределами циклов.
    # Это позволяет избежать повторного копирования из pin_memory в обычную CPU-память.
    audio_16k_cpu = batch['audio_16k'].cpu()
    audio_cpu = batch['audio'].cpu()

    # ------------------ Voice Encoder Embeds (ve_embeds) ------------------------
    ve_embeds = []
    for i in range(B):
        try:
            # 🔨 ИЗМЕНЕНИЕ: Вызываем .numpy() только на уже извлеченном CPU-тензоре
            ...
            # ⚡️ ОПТИМИЗАЦИЯ 2: Сразу преобразуем результат в тензор и отправляем на GPU
            parts = torch.from_numpy(utt_embeds).to(device)
            
            # Последующая логика на GPU
            ...
            # print(f"Error in voice embedding {i}: {e}") # Отладочный вывод
            if ve_embeds:
                # Клонирование последнего успешного эмбеддинга (уже на GPU)
                ve_embed = ve_embeds[-1].clone() 
            else:
                # Создание запасного эмбеддинга СРАЗУ на GPU
                ve_embed = torch.zeros(1, 256, device=device) 
            ve_embeds.append(ve_embed)

    # ⚡️ ОПТИМИЗАЦИЯ 3: Конкатенация эмбеддингов (все уже на GPU)
    ve_embeds = torch.cat(ve_embeds, dim=0)


    # ------------------ S3Gen Reference Embeds (s3gen_refs) ---------------------
    s3gen_refs = []
    for i in range(B):
        try:
            # 🔨 ИЗМЕНЕНИЕ: Используем уже извлеченный CPU-тензор
            audio_np = audio_cpu[i].numpy()
            ref_audio = audio_np[:model.DEC_COND_LEN]
            
            # Предполагаем, что embed_ref принимает device и размещает результат на нем.
            s3gen_refs.append(s3gen.embed_ref(ref_audio, S3GEN_SR, device=device))
        except Exception as e:
            # print(f"Error in S3Gen ref {i}: {e}") # Отладочный вывод
            # Создание запасного эмбеддинга СРАЗУ на GPU
            s3gen_refs.append(s3gen.embed_ref(np.zeros(model.DEC_COND_LEN), S3GEN_SR, device=device))

    # ------------------ T3 Condition Tokens (t3_cond_tokens) --------------------
    ...
    if plen:
        for i in range(B):
            try:
                ...
                # ⚡️ ОПТИМИЗАЦИЯ 4: Сразу на GPU
                tok_list.append(torch.atleast_2d(tokens).to(device))
            except Exception as e:
                # print(f"Error tokenizing speech {i}: {e}") # Отладочный вывод
                # Create dummy tokens СРАЗУ на GPU
                dummy_tokens = torch.zeros(1, plen, dtype=torch.long, device=device)
                tok_list.append(dummy_tokens)
        
        # ⚡️ ОПТИМИЗАЦИЯ 5: Конкатенация токенов (все уже на GPU)
        t3_cond_tokens = torch.cat(tok_list, dim=0)
    else:
        # Создание пустого тензора СРАЗУ на GPU
...
def compute_loss(
    ...
    target_tokens_list = []
    for i in range(batch_size):
        # 🔨 ИЗМЕНЕНИЕ: Используем уже извлеченный CPU-тензор, вызывая только .numpy()
        audio_16k_np = audio_16k_cpu[i].numpy()
...
def main():
    ...
    # Load audio samples
    samples = load_audio_samples(AUDIO_DATA_DIR)
    if len(samples) == 0:
    ...
    try:
        ...
        raise
    # Restart training
    #model = ChatterboxMultilingualTTS.from_local("./checkpoints_lora/merged_model", DEVICE)

    # ------------------ ⚡️ ОПТИМИЗАЦИЯ: Gradient Checkpointing ⚡️ ------------------
    try:
        if DEVICE == 'cuda':
            # Этот код предполагает, что у T3-модели есть метод для включения GC
            # или что она основана на Hugging Face Transformers.
            # В HF моделях это выглядит так:
            # model.t3.tfmr.gradient_checkpointing_enable()
            
            # Если вы используете модель Chatterbox, вам может понадобиться
            # найти способ включить GC для внутренних блоков Transformer'а.
            
            # Если T3 — это просто nn.Module, то используем универсальный подход:
            
            # Универсальная функция для применения GC к модулям с именем 'LlamaDecoderLayer'
            def set_gradient_checkpointing(model, value=True):
                for name, module in model.named_modules():
                    # Ищем слои декодера/блока, где хранятся активации
                    if 'LlamaDecoderLayer' in module.__class__.__name__ or 'TransformerBlock' in module.__class__.__name__:
                        if hasattr(module, 'gradient_checkpointing'):
                            module.gradient_checkpointing = value
                        elif hasattr(module, 'use_gradient_checkpointing'):
                             module.use_gradient_checkpointing = value
                        else:
                            # Если не найдено стандартных флагов, можно попробовать установить флаг 
                            # на верхнем уровне, который поддерживается PyTorch/HF
                            if hasattr(model, 'gradient_checkpointing_enable'):
                                 model.gradient_checkpointing_enable()
                                 return
                
                # Если не удалось найти, выводим предупреждение
                if not hasattr(model, 'gradient_checkpointing_enable') and value:
                    print("⚠️ Checkpointing не включен. Необходимо найти правильные внутренние блоки Transformer'а.")

            # Пытаемся включить Checkpointing
            set_gradient_checkpointing(model.t3.tfmr, True)
            print("✅ Gradient Checkpointing включен (снижение VRAM).")

    except Exception as e:
        print(f"⚠️ Ошибка при включении Checkpointing: {e}")
    # --------------------------------------------------------------------------------
    
    # Определяем фактический размер словаря токенизатора
    new_vocab_size = len(model.tokenizer.tokenizer.get_vocab())
    print(f"Новый размер словаря токенизатора: {new_vocab_size}")
    # Получаем текущий размер слоя встраивания
    old_embedding_layer = model.t3.text_emb
    old_vocab_size = old_embedding_layer.num_embeddings
    print(f"Текущий размер слоя встраивания: {old_vocab_size}")

    if new_vocab_size != old_vocab_size:
        print(f"⚠️ Размеры не совпадают. Изменяем размер слоя встраивания...")
        
        # Создаем новый слой nn.Embedding с правильным размером
        new_embedding_layer = nn.Embedding(
            num_embeddings=new_vocab_size, 
            embedding_dim=old_embedding_layer.embedding_dim 
        )

        # Копируем веса для старых токенов
        new_embedding_layer.weight.data[:old_vocab_size] = old_embedding_layer.weight.data

        # Инициализируем новые веса для новых токенов
        nn.init.normal_(
            new_embedding_layer.weight.data[old_vocab_size:], 
            mean=0.0, 
            std=0.01
        )

        # Заменяем старый слой новым
        model.t3.text_emb = new_embedding_layer.to(DEVICE)
        print("✅ Размер слоя встраивания успешно изменен.")
    else:
        print("Размеры словаря и слоя встраивания совпадают.")
    ...
    merged_model = ChatterboxMultilingualTTS.from_local("./models/original_model", DEVICE)
    ...
    # ⚠️ КРИТИЧЕСКИ ВАЖНЫЙ ШАГ: КОПИРОВАНИЕ ИЗМЕНЕННОГО СЛОЯ EMBEDDING
    # Проверяем, был ли размер изменен в обученной модели (model), 
    # и копируем его в модель для сохранения (merged_model).

    trained_text_emb = model.t3.text_emb
    merged_text_emb = merged_model.t3.text_emb

    if trained_text_emb.num_embeddings != merged_text_emb.num_embeddings:
        print(f"🔄 Обновление слоя text_emb: копирование измененного словаря ({trained_text_emb.num_embeddings}) в merged_model.")
        # Копируем весь объект nn.Embedding, который был изменен в начале main()
        merged_model.t3.text_emb = trained_text_emb.to(DEVICE)
    else:
        print("✅ Слой text_emb не требовал обновления (размер совпадает).")
    ...
    save_file(merged_model.t3.state_dict(), merged_dir / "t3_mtl23ls_v2.safetensors") # <-- ИЗМЕНЕНИЕ
    ...
    print(f"  model = ChatterboxMultilingualTTS.from_local('{merged_dir}', device='{DEVICE}')")
...
def load_audio_samples(audio_dir: str, ) -> List[AudioSample]:
   ...
   samples.sort(key=lambda sample: sample.duration, reverse=True)
...
def merge_lora_weights(model: ChatterboxMultilingualTTS, lora_layers: Dict[str, LoRALayer]):
...
def load_lora_adapter(model: ChatterboxMultilingualTTS, filepath: str, device: str = 'cuda'):
