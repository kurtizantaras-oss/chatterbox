...
CHECKPOINT_PATH = "./checkpoints_lora/checkpoint_epoch3_step1644.pt"  # Path to your checkpoint
...
def save_merged_model(model: ChatterboxMultilingualTTS, output_dir: Path):
    ...
    save_file(model.t3.state_dict(), output_dir / "t3_mtl23ls_v2.safetensors") # <-- ИЗМЕНЕНИЕ
...
def main():
    ...
    print(f"  model = ChatterboxMultilingualTTS.from_local('{OUTPUT_DIR}', device='{DEVICE}')")
...
def verify_merged_model(output_dir: str):
    ...
    try:
        model = ChatterboxMultilingualTTS.from_local(output_dir, DEVICE)
